    ...
    override open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.row >= prefixDays {
            ...
            if arrSelectedDates.filter({ $0.isDateSameDay(currentDate)
                ...
                if (currentDate.isGreaterThanDat(dateToCompare: nextMonthFirstDay)) {
                ...
                if startDate != nil {
                    if Calendar.current.startOfDay(for: cell.currentDate as Date).isLessThanDat(dateToCompare: Calendar.current.startOfDay(for: startDate!)) {
                        cell.isCellSelectable = false
                        cell.lblDay.textColor = self.dayDisabledTintColor
                    }
                }
